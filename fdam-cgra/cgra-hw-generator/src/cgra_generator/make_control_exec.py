...
def make_control_exec(num_pe, num_pe_io, net_radix):
    ...
    en_pc_net = m.OutputReg('en_pc_net', 1)
    ...
    en_process = m.Reg('en_process', num_pe + en_net_bits + 1)
    en_process1 = m.Reg('en_process1', num_pe + en_net_bits + 1)
    ...
    m.Always(Posedge(clk))(
        If(rst)(
            ...
            en_pc_net(en_process1[en_process1.width - 1])
        )
    )
...
# make_control_exec(128, 32, 4).to_verilog('control_exec')
