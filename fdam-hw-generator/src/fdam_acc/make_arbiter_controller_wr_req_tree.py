...
def make_arbiter_controller_wr_req_tree(radix, num_input):
    ...
    m = Module('fdam_arbiter_controller_wr_req_tree_%d' % num_input)
    ...
    if len(array) == 1:
        ...
        if not num_input in mods.keys():
            mods[num_input] = make_arbiter_controller_wr_req(num_input)
        m.Instance(mods[num_input], 'arbiter_controller_wr_req_%d_%d' % (0, 0), m.get_params(), con)
    else:
        ...
        for i in range(len(array) - 1):
            ...
            for j in range(len(array[i])):
                ...
                if not array[i][j] in mods.keys():
                    mods[array[i][j]] = make_arbiter_controller_wr_req(array[i][j])
                m.Instance(mods[array[i][j]], 'arbiter_controller_wr_req_%d_%d' % (i, j), m.get_params(), con)
                id_out = id_out + 1
...
def make_arbiter_controller_wr_req(num_input):
    m = Module('fdam_arbiter_controller_wr_req_%d' % num_input)
    NUM_INPUT = num_input
    DATA_WIDTH = m.Parameter('DATA_WIDTH', 32)
    ...
    IN_FIFO_ALMOSTFULL_THRESHOLD = m.Localparam('IN_FIFO_ALMOSTFULL_THRESHOLD',
                                                EmbeddedCode('2 ** INPUT_FIFO_DEPTH_BITS - 8'))
    ...
    m.EmbeddedCode('')
    rst_reg = m.Wire('rst_reg', 8 + NUM_INPUT)
    m.EmbeddedCode('')
    ...
    m.Instance(reg_tree, reg_tree.name, param, con + con1)
    ...
    con = [('clk', clk), ('rst', rst_reg[idx]), ('we', in_fifo_we[idx]),
    ...
    con = [('clk', clk), ('rst', rst_reg[NUM_INPUT]), ('data_in_valid', in_fifo_dout_valid)]
    ...
    con = [('clk', clk), ('rst', rst_reg[NUM_INPUT + 1]), ('we', out_fifo_we), ('din', out_fifo_din),
           ('re', out_fifo_re),
    ...
    con = [('clk', clk), ('rst', rst_reg[NUM_INPUT + 2]), ('request', arbiter_request),
    ...
    gen.Assign(arbiter_request[idx](~in_fifo_almostempty[idx] & ~out_fifo_almostfull))

    m.Always(Posedge(clk))(
        If(rst_reg[NUM_INPUT + 3])(
    ...
    m.Always(Posedge(clk))(
        If(rst_reg[NUM_INPUT + 7])(
            ...
            Case(fsm_in_fifo_re)(
                ...
                When(Int(2, fsm_in_fifo_re.width, 10))(
                    in_fifo_re((Int(0, NUM_INPUT, 10))),
                    fsm_in_fifo_re(Int(3, fsm_in_fifo_re.width, 10))
                ),
                When(Int(3, fsm_in_fifo_re.width, 10))(
                    in_fifo_re((Int(0, NUM_INPUT, 10))),
                    fsm_in_fifo_re(Int(0, fsm_in_fifo_re.width, 10))
                ),
                When()()
