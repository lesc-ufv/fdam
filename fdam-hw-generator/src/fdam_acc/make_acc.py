...
def make_acc(acc_param, input_queue_controller, output_queue_controller, dsm_controller, accelerator_module=None):
    ...
    m = Module('fdam_accelerator_%d' % acc_id)
    ...
    acc_user_done_reg = m.Wire('acc_user_done_reg', num_output_queues)
    ...
    reg_tree = make_reg_tree_pipeline(4, acc_user_done_reg.width, 4)
    param = [('DATA_WIDTH', 1)]
    con = [('clk', clk), ('in', acc_user_done)]
    con1 = [('out_%d' % i, acc_user_done_reg[i]) for i in range(acc_user_done_reg.width)]
    m.Instance(reg_tree, 'acc_done_' + reg_tree.name, param, con + con1)
