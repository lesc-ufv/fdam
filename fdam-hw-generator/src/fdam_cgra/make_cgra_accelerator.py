...
def make_cgra_accelerator(cgra_id, num_pe, num_pe_io, data_width, net_radix, mem_conf_depth):
    net = make_omega(num_pe * 2, 0, net_radix, mem_conf_depth)
    en_net_bits = net.get_ports().get('en').width
    en_pc_net_bits = net.get_ports().get('en_pc_net').width
    num_cicle_wait_conf_finish = num_pe + en_pc_net_bits

    conf_bus_width = 64

    fd = make_fecth_data()
    dd = make_dispath_data()
    control_conf = make_control_conf(cgra_id, num_pe_io, num_cicle_wait_conf_finish)
