...
def make_cgra(cgra_id, num_pe, num_pe_in, num_pe_out, data_width, net_radix, conf_depth):
    ...
    branch_stack_depth = 4

    pes_en = m.Input('pes_en', num_pe)
    conf_bus_in = m.Input('conf_bus_in', conf_bus_width)

    net = make_omega(num_thread, num_pe * 2, 0, net_radix, conf_depth)
    ...
    conf_bus = [m.Wire('conf_bus_%d' % i, conf_bus_width) for i in range(num_pe + 1)]
    ...
    memory = make_memory()
    pe_in = make_pe(cgra_id, 1, num_thread, branch_stack_depth, pc, memory, reg_pipe, data_width, conf_depth)
    pe_out = make_pe(cgra_id, 2, num_thread, branch_stack_depth, pc, memory, reg_pipe, data_width, conf_depth)
    pe = make_pe(cgra_id, 0, num_thread, branch_stack_depth, pc, memory, reg_pipe, data_width, conf_depth)
