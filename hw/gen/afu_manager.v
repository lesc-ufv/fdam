...
(
  ...
  wire [16-1:0] req_wr_en_in;
  wire [(DATA_WIDTH+ADDR_WIDTH+TAG_WIDTH)*16-1:0] req_wr_data_in;
  wire [16-1:0] req_wr_available_in;
  ...
  (
    ...
    .available_write(req_wr_available_in[15:0]),
    .request_write(req_wr_en_in[15:0]),
    .write_data(req_wr_data_in[16*(0*(DATA_WIDTH+ADDR_WIDTH + TAG_WIDTH)+(DATA_WIDTH+ADDR_WIDTH + TAG_WIDTH))-1:16*(0*(DATA_WIDTH+ADDR_WIDTH + TAG_WIDTH))]),
    .write_data_valid(resp_wr_valid),
    .write_queue_id(resp_wr_mdata)
  );


  arbiter_controller_top_16
  #(
