...
def make_arbiter_controller_tree(radix, num_input):
    array = []
    array = make_arbiter_controller_tree_array(radix, num_input, array)
    m = Module('arbiter_controller_top_%d' % num_input)
    ...
    if len(array) == 1:
        ...
        if not num_input in mods.keys():
            mods[num_input] = make_arbiter_controller(num_input)
        m.Instance(mods[num_input], 'arbiter_controller_%d_%d' % (0, 0), m.get_params(), con)
    else:
        ...
        for i in range(len(array) - 1):
            ...
            for j in range(len(array[i])):
                ...
                if not array[i][j] in mods.keys():
                    mods[array[i][j]] = make_arbiter_controller(array[i][j])
                m.Instance(mods[array[i][j]], 'arbiter_controller_%d_%d' % (i, j), m.get_params(), con)
                id_out = id_out + 1
        ...
        if not array[len(array) - 1][0] in mods.keys():
            mods[array[len(array) - 1][0]] = make_arbiter_controller(array[len(array) - 1][0])
        m.Instance(mods[array[len(array) - 1][0]], 'arbiter_controller_%d_%d' % (len(array) - 1, 0), m.get_params(),
                   con)
    return m
...
def make_arbiter_controller(num_input):
    m = Module('arbiter_controller_%d' % num_input)
    NUM_INPUT = num_input
    DATA_WIDTH = m.Parameter('DATA_WIDTH', 32)
