...
def make_afu_manager(afus):
    try:
        ...
        NUM_CL_DSM_RD = m.Localparam('NUM_CL_DSM_RD',int(ceil(qtd_in_queue/8)))
        NUM_CL_DSM_WR = m.Localparam('NUM_CL_DSM_WR',int(ceil(qtd_out_queue/8)))
        NUM_CL_DSM_TOTAL = m.Localparam('NUM_CL_DSM_TOTAL', EmbeddedCode('1 + NUM_CL_DSM_RD + NUM_CL_DSM_WR'))
        NUM_CL_DSM_TOTAL_BITS = m.Localparam('NUM_CL_DSM_TOTAL_BITS', numBits(1 + NUM_CL_DSM_RD.value + NUM_CL_DSM_WR.value))
        DSM_ADDR_WIDTH = m.Localparam('DSM_ADDR_WIDTH', NUM_CL_DSM_TOTAL_BITS)
        DSM_DATA_WIDTH = m.Localparam('DSM_DATA_WIDTH', DATA_WIDTH)
        DSM_NUM_CL = m.Localparam('DSM_NUM_CL', NUM_CL_DSM_TOTAL)
        ...
        for num_in_queue, num_out_queue in afus:
            ...
            params = [('ADDR_WIDTH', ADDR_WIDTH), ('QTD_WIDTH', QTD_WIDTH), ('DATA_WIDTH', DATA_WIDTH),
                      ...
                      ('FIFO_DEPTH_BITS', FIFO_DEPTH_BITS), ('FIFO_FULL', FIFO_FULL),('DSM_DATA_WIDTH',DSM_DATA_WIDTH),
                      ...
                      ('DSM_ADDR_WIDTH', DSM_ADDR_WIDTH),  ('DSM_NUM_CL', DSM_NUM_CL)]
