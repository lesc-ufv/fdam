...
try:
    ...
    sys.path.insert(0, FAM_BASEDIR + '/hw/fam-generator')
except:
    print("\033[1;31m")
...
from components.make_acc_management import make_acc_management
from scripts.fam_project_utils import *
...
def create_fam_project():
    ...
    while True:
        prj_path = input('Project path[%s/samples ]: ' % FAM_BASEDIR)
        if prj_path == '':
            prj_path = FAM_BASEDIR + '/samples'
...
def main():
    try:
        ...
        create_cmake_file(path_for_project)
        create_main_file(path_for_project)
