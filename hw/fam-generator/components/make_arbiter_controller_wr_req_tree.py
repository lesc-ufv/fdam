...
from components.make_arbiter import make_arbiter
from components.make_fifo import make_fifo
from components.make_select import make_select_tree
from scripts.utils import numBits


def make_arbiter_controller_wr_req_tree(radix, num_input):
    array = []
    array = make_arbiter_controller_tree_array(radix, num_input, array)
    m = Module('arbiter_controller_wr_req_tree_%d' % num_input)
...
def make_arbiter_controller_wr_req(num_input):
    m = Module('arbiter_controller_wr_req_%d' % num_input)
    NUM_INPUT = num_input
    DATA_WIDTH = m.Parameter('DATA_WIDTH', 32)
