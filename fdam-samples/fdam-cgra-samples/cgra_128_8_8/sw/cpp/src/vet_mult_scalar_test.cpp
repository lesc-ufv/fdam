...
//
#include <vet_mult_scalar_test.h>
...
int vet_mult_scalar_test(int argc, char *argv[]) {
    ...
    vet_mult_scalar_cpu(num_threads, data_in, cpu_data_out, data_num, const_vet);
    ...
    vet_mult_scalar_cgra(num_threads, data_in, cgra_data_out, num_threads * data_num);
...
void vet_mult_scalar_cpu(int num_threads, const short *data_in, short *data_out, int n, short constant) {
    for (int j = 0; j < num_threads; j++) {
        for (int i = 0; i < n; ++i) {
            data_out[i * num_threads + j] = (data_in[i * num_threads + j] * (short)(j+1));
        }
    }
}

void vet_mult_scalar_cgra(int num_thread, short *data_in, short *data_out, int n) {
    cgra_program_t *cgra_program = create_vet_mult_scalar_cgra_program_from_file(num_thread,
                                                                                 "../cgra_programs/vet_mult_scalar.cgra",
...
cgra_program_t *
create_vet_mult_scalar_cgra_program_from_file(int num_thread, const char *file_path, short *data_in, int data_in_size,
                                              short *data_out,
                                              int data_out_size) {
